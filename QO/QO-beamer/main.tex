\documentclass[12pt,UTF8,aspectratio=169]{beamer} 
%aspectratio=1610：160mm，100mm
%aspectratio=169：160mm，90mm
%aspectratio=43：128mm，96mm，默认尺寸
\RequirePackage{mybeamer}
%\includeonlyframes{current} % 只编译 \begin{frame}[label=current] \end{frame}
\usepackage{overpic}
%%% sudo vim /usr/local/texlive/2021/texmf-dist/tex/xelatex/mybeamer
%%%% sudo texhash

%-------------------正文-------------------------%
%                                               %
%                                               %
\begin{document}                                %
%                                               %
%                                               %
%-----------------------------------------------%

%题目，作者，学校，日期                
\author{李小飞}
\title{\textbf{\Huge 量~子~光~学}}
\subtitle{Quantum Optics}
\institute[电子科技大学]{光电科学与工程学院}
\date{\today}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\frame[plain]{\titlepage}
\begin{frame} [plain]
    \setbeamertemplate{headline} {} %
    \Background[17] 
    \maketitle
    \addtocounter{framenumber}{-1} 
\end{frame}
%\maketitle
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%-----------章节-------------------------% 
%\include{tmp1.tex}

%\include{lecture_1}        % 半经典光
%\include{lecture_2and3}    % 量子力学基础与谐振子
%\include{lecture_4and5}    % 光场量子化
%\include{lecture_6and7}    % 相干态
%\include{lecture_8and9}    % 压缩态
%\include{lecture_10and11}  % 光子探测与计数
%\include{lecture_12and13}  % 光场统计 关联函数
%
\include{lecture_14and15}  % 光场表示
%\include{lecture_16and17}  % 光与原子的相互作用
%\include{lecture_18and19}  % 光腔中的原子
%\include{lecture_20}       % 原子冷却
%--------------------------------------%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}[plain]
    \Background[2] 
	\begin{center}
		{\huge \color{deepred} \textrm{Thanks for your attention!  \\ \vspace{1.0em}
         A \& Q}}
	\end{center}
    \addtocounter{framenumber}{-1} 
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}
%---------------THE END-----------------%
